...
public abstract class TMNController : MonoBehaviour 
{
	...
	public virtual void Start()
	{
		if (map == null)
		{
			MessageBox.DEBUG("The 'map' property was not set, attempting to find a MapNav in the scene.");
	...
	protected void HandleInput()
	{
        ...
        if (Physics.Raycast(ray, out hit, 500f))    //, map.tilesLayer))
        {
            ...
            {	// mouse-click/touch detected
                ...
                else if (hit.collider.gameObject.layer == map.unitsLayer)
                {
                    unselect = false;
