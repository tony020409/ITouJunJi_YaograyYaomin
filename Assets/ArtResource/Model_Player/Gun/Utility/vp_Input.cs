...
public class vp_Input : MonoBehaviour
{
	...
	public List<string> ButtonKeys2 = new List<string>();
	...
	public virtual void AddSecondaryButton(string n, KeyCode k = KeyCode.None)
	{
		...
		if (ButtonKeys2.Contains(n))
			ButtonValues2[ButtonKeys2.IndexOf(n)] = k;
		else
		{
			ButtonKeys2.Add(n);
			ButtonValues2.Add(k);
	...
	public virtual void UpdateDictionaries()
	{
		...
		Buttons2.Clear();
		for (int i = 0; i < ButtonKeys2.Count; i++)
		{
			if (!Buttons2.ContainsKey(ButtonKeys2[i]))
				Buttons2.Add(ButtonKeys2[i], ButtonValues2[i]);
		}
	...
	public static void ChangeButtonKey( string button, KeyCode keyCode, bool save = false )
	{
		...
		{
			if (save)
				Instance.ButtonValues2[vp_Input.Instance.ButtonKeys2.IndexOf(button)] = keyCode;
	...
	public static void ChangeAxis(string n, KeyCode pk = KeyCode.None, KeyCode nk = KeyCode.None, bool save = false)
	{
		...
		{
			MessageBox.DEBUG("The Axis \"" + n + "\" Doesn't Exist");
			return;
		}
