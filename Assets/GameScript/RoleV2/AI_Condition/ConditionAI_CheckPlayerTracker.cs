...
public class ConditionAI_CheckPlayerTracker : AI_ConditionBaseStateV2
{


    public ConditionAI_CheckPlayerTracker()
        : base(AI_EM.EM_AIState.CheckPlayerTracker)
    { }
    ...
    public override bool f_ConditionTest()
    {
        ...
        for (int i = 0; i < aData.Count; i++)
        {
            ...
            {
                if (Vector3.Distance(aData[i].GetComponent<MySelfPlayerControll2>().m_oOther1.transform.position, _BaseRoleControl.transform.position) < _BaseRoleControl.f_GetAttackSize())
                {
            ...
            {
                if (Vector3.Distance(aData[i].GetComponent<OtherPlayerControll2>().m_oOther1.transform.position, _BaseRoleControl.transform.position) < _BaseRoleControl.f_GetAttackSize())
                {
                    return ReturnOwner(_BaseRoleControl.m_iId, aData[i].m_iId);
                }
            }
