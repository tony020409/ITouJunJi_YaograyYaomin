...
public class GameControllChangeGun : GameControllBaseState
{
    ...
    public GameControllChangeGun(): 
    base((int)EM_GameControllAction.ChangeWeapon){}
        ...
        if (IsRuning())
        {
            ...
            { 
                for (int i = 0; i < tRole.Count; i++)
                {
                    ...
                    {
                        tRole[i].GetComponent<MySelfPlayerControll2>().f_ChangeWeapon(_CurGameControllDT.szData2); //角色換槍
                    }
                    if (i == tRole.Count-1)
                    {
                        EndRun();
